    ...
    <div className="space-y-6">
      ...
      <section className="grid sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-border/50 p-5">
          ...
          <div className="text-sm text-muted-foreground">Tổng vận đơn</div>
        </div>
        <div className="bg-white rounded-xl border border-border/50 p-5">
      ...
      <section className="bg-white rounded-2xl border border-border/50 overflow-hidden">
        <div className="px-6 py-4 border-b border-border">
          <h2 className="text-base font-semibold font-display">Danh sách vận đơn</h2>
        </div>

        {orders.length === 0 ? (
          <div className="p-8 text-sm text-muted-foreground text-center">Chưa có dữ liệu vận đơn.</div>
        ) : (
              ...
              return (
                <article key={order.id} className="p-6 space-y-2">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        {order.origin} - {order.destination} • {order.user.profile?.fullName || order.user.name || 'Khách hàng'}
                      </div>
                    </div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Tracking: {latestEvent ? `${latestEvent.location} - ${formatDateTime(latestEvent.eventTime)}` : 'Chưa có'}
                  </div>
                  <form action={updateOrderStatusAction} className="pt-2 border-t border-border/70 grid gap-2 lg:grid-cols-4">
                    ...
                    <input
                      type="text"
                      name="description"
                      placeholder="Ghi chú tracking (tùy chọn)"
                      className="h-9 rounded-lg border border-border px-3 text-sm"
                    />
                    <button
                      type="submit"
