  ...
  return (
    <div className="space-y-6">
      <section className="bg-white rounded-2xl border border-border/50 p-6">
        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground mb-2">Tuyến đường</p>
        <h1 className="text-2xl font-bold font-display">Hiệu suất theo tuyến</h1>
        <p className="text-sm text-muted-foreground mt-2">Tổng hợp theo cặp điểm gửi - điểm nhận từ dữ liệu vận đơn thực tế.</p>
      </section>

      <section className="grid lg:grid-cols-5 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-border/50 p-6 space-y-4">
          <h2 className="text-base font-semibold font-display">Top 5 tuyến</h2>
          {topFive.length === 0 ? (
            ...
            topFive.map((route) => (
              <div key={`${route.origin}-${route.destination}`} className="rounded-xl bg-muted/30 p-4">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  {route._count._all} chuyến • {formatCurrencyVnd(route._sum.totalAmount ?? 0)}
                </div>
              </div>
            ))
          )}
        ...
        <div className="lg:col-span-3 bg-white rounded-2xl border border-border/50 overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            <h2 className="text-base font-semibold font-display">Tất cả tuyến</h2>
            <Route className="w-4 h-4 text-muted-foreground" />
          </div>

          {routes.length === 0 ? (
            <div className="p-8 text-sm text-muted-foreground text-center">Chưa có dữ liệu tuyến đường.</div>
          ) : (
              ...
              {routes.map((route) => (
                <div key={`${route.origin}-${route.destination}`} className="p-5 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mt-1">{route._count._all} chuyến</div>
                  </div>
